...
int32_t encryptContent(int32_t nodes, char *content_filename, char * revokedset)
{
	...
	// TODO - Cambiar el método, no pilla números de 2 cifras
	...
	char* filename = /*new char[strlen(content_filename) + 12];*/ "encrypted";
...
int32_t decryptContent(char * ciphered_content_filename, int32_t device)
{
	...
	char* output_filename = /*new char[strlen(ciphered_content_filename) + 12];*/ "decrypted";
...
int32_t aes_encrypt_func(byte* plaintext, int32_t plaintext_len, const byte* key, const byte* iv, byte* ciphertext)
{
	...
	if (!ctx)
	{
		std::cout << "Error while initializing engine\n";
		return -1;
	}
...
{
	return index > 0 && (uint32_t)index < tree.size();
}
...
int readContentFromFile(const char * filename, byte*& buffer)
{
	...
	// Buffer
	buffer = new byte[content_length + 1];
